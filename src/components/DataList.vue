    ...
    <el-table :data="tableData" stripe fit>
      ...
      <el-table-column prop="type" label="载体"> </el-table-column>
       <el-table-column prop="text" label="描述">
        ...
        　　　　　　　　　　<span v-if="scope.row.type=== '文字'">{{scope.row.text}}</span>
        　　　　　　　　　　<span v-if="scope.row.type=== '图像'"><a :href="'http://localhost:8080/'+scope.row.media">点击下载图片</a></span>
                        　<span v-if="scope.row.type=== '视频'"><a :href="'http://localhost:8080/'+scope.row.media">点击查看视频</a></span>
        　　　　　　　　</template>
      </el-table-column>
    </el-table>
    </el-card>
